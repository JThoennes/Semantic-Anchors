...
Tests derived from the specification verify the rule as it was written.
They miss the property behind the rule.
A rule such as "all shares are equal" makes weighting invisible: a half share counted as a full one still produces a plausible number, and every example-based test stays green.
...
The feedback loop above asks a forward question: what did we learn?
An agent that learned nothing new answers "no" truthfully, and a statement that was already wrong stays in the document.
Drift therefore survives any number of review steps.
It is not caught by asking for news; it is caught by asking whether a specific claim still holds.
...
The prohibition in the middle paragraph carries the weight.
...
When the loop ends, run the documentation verification from Step 8 once more, against the code that now exists.
