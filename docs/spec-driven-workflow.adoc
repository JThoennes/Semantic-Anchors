...
[NOTE]
====
*Working on an existing codebase?* This page describes the greenfield workflow — starting from a blank slate. For brownfield projects (existing code, missing or untrusted documentation), see the link:#/brownfield[Brownfield Workflow] and the installable link:#/socratic-recovery-skill[Socratic Code-Theory Recovery Skill].
====
...
This workflow was used to build four open source projects, all 100% AI-generated.
...
* https://doctoolchain.org/Bausteinsicht/[*Bausteinsicht*]: Bidirectional synchronization between a structured JSON architecture model and draw.io diagrams — Architecture-as-Code meets draw.io.
...
Tests derived from the specification check the rule as written.
They do not check the property the rule rests on.

When every input in the examples happens to be equal, a calculation that correctly weights its inputs and one that merely counts them return the same number.
Both pass. The bug appears the first time an input differs, which is usually after release.
...
The second question is the one that pays.
It checks the test suite against changes the specification never anticipated, which is where example-based tests are blind by construction.


==== Feedback Loop to Specification

Documentation drifts from code in two directions, and each needs its own question.

It falls behind when implementation discovers something: a decision taken, a rule clarified, a constraint found.
It also goes stale in place: a module gets renamed, a column disappears, a capability stays described that was never built.
The second kind survives a question about what you learned, because you learned nothing -- the sentence was already wrong when you last read past it.

Periodically ask the AI both:
...
The prohibition in the second question carries weight.
The document describes an intention, the code describes reality; adapting the code to the document turns a documentation error into a silent behavior change.

Structural claims can also be checked mechanically, which is cheaper and more reliable than asking an agent each time:
...
Traceability tooling usually runs forward only: it enforces that every rule has a test.
That direction never notices a documented column the schema does not have.
...
When the loop ends, run the two documentation questions from Step 8 once more, against the code that now exists.
This is the point where it pays most: bug fixes change behavior, and changed behavior is what turns a correct structural claim into a stale one.
Do it before the release, not after, or the first reader of the documentation will be the first person to find the drift.
