...
*Single-run observations, recorded as such:* one weak-tier P1 run for each candidate reported `reaching=yes`. Neither reproduced in its second run, so neither is treated as a finding.
