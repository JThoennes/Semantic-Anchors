...
*New: measured anchors now say which models they were verified on:*

* Anchors carry a `:verified-on:` attribute, and the modal renders it at the foot of the entry, linked to the link:#/prior-tests[prior-test register]. The tier stays out of the interface deliberately: a tier is a judgement, a model list is a fact, and the fact is what gives a reader confidence.
* The field records where an anchor *delivered*, never merely where it was tried. *link:#/anchor/premortem[Premortem]*, *link:#/anchor/second-order-thinking[Second-Order Thinking]* and *link:#/anchor/locality-of-behaviour[Locality of Behaviour]* list all three tiers; *link:#/anchor/simplified-technical-english[Simplified Technical English]* lists only two, because on a weak model its rewrite was not distinguishable from an unanchored one. Listing it there would have claimed a result the measurement did not produce.
