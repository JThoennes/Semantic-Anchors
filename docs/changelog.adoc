...
* The workflow page (EN + DE) gains _Invariant Tests_ and _Verifying the Documentation Against the Code_ under Step 8, each with its prompt, and Step 12 now ends by running the verification once more against the code that exists after the bug-fix loop — the point where it pays most, since fixes change behaviour and changed behaviour is what makes a correct structural claim stale. The existing feedback loop is kept: asking what we learned is still useful, it is just not sufficient on its own.
