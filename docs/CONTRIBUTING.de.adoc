...
* `:tier:` - Aktivierungsstufe: `1` (nur Frontier-Modelle), `2` (braucht Qualifizierung), `3` (breit erkannt). Intern: ein Urteil, deshalb bewusst nicht auf der Seite dargestellt
* `:prior-test:` - Datum, an dem der Anker mit dem link:#/anchor-prior-test[Anchor Prior Test] gemessen wurde. Der Schlüssel ins link:#/prior-tests[Prior-Test-Register]; fehlt das Attribut, heißt das „nicht gemessen“ -- eine Aussage über unsere Aufzeichnungen, nicht über den Begriff
* `:tested-on:` - Jedes Modell, gegen das der Anker gemessen wurde, mit Ergebnis, als `modell=yes|no`-Paare (z.B. `haiku-4.5=no, sonnet-5=yes, opus-5=yes`). Wird im Modal angezeigt. *Alle* getesteten Modelle eintragen, nicht nur die erfolgreichen: Ein Modell, das geprüft wurde und keine Wirkung zeigte, ist eine andere Aussage als eines, das nie geprüft wurde -- lässt man es weg, liest es sich wie Letzteres
...
Die Section wegzulassen heißt nicht, das Ergebnis wegzuwerfen. Den Negativbefund mit Datum und abgesuchten Sprachen im link:#/prior-tests[Prior-Test-Register] festhalten, damit später unterscheidbar bleibt, ob jemand recherchiert und nichts gefunden hat oder ob nie jemand nachgesehen hat. Ein „nichts gefunden" ohne Suchrahmen ist kein Beleg für Abwesenheit — dazugehört, was abgesucht wurde und was nicht erreichbar war; eine unerreichbare Quelle nie als belegte Abwesenheit ausgeben.
