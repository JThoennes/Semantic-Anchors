...
Das Modell meldet drei Felder selbst, und auf das dritte kommt es an. `recognized` sagt, ob es den Begriff überhaupt kennt. `richness` sagt, wie viele vernetzte Konzepte es hervorgebracht hat. *`reaching` sagt, ob das Modell gegriffen hat* -- ob es aus den Wörtern des Begriffs eine plausible Antwort rekonstruiert, statt ein Konzept abzurufen, das es besitzt. Eine solche Antwort sieht oft gut aus; erkennbar wird sie an generischem Inhalt und fehlenden Einzelheiten, etwa daran, dass weder Urheber noch Fachgebiet genannt werden. Greift die schwache Stufe reproduzierbar ins Ungefähre, ist das der früheste klare Hinweis auf einen dünnen Prior.
