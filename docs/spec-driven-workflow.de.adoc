...
[NOTE]
====
*Bestehender Codebase?* Diese Seite beschreibt den Greenfield-Workflow — Start auf der grünen Wiese. Für Brownfield-Projekte (existierender Code, fehlende oder unzuverlässige Dokumentation) siehe den link:#/brownfield[Brownfield-Workflow] und den installierbaren link:#/socratic-recovery-skill[Socratic Code-Theory Recovery Skill].
====
...
Dieser Workflow wurde verwendet, um vier Open-Source-Projekte zu bauen, alle 100% KI-generiert.
...
* https://doctoolchain.org/Bausteinsicht/[*Bausteinsicht*]: Bidirektionale Synchronisation zwischen einem strukturierten JSON-Architekturmodell und draw.io-Diagrammen — Architecture-as-Code trifft draw.io.
...
Tests, die aus der Spezifikation abgeleitet sind, prüfen die Regel so, wie sie dasteht.
Die Eigenschaft, auf der die Regel ruht, prüfen sie nicht.

Sind in den Beispielen zufällig alle Eingaben gleich groß, liefern eine Berechnung, die ihre Eingaben korrekt gewichtet, und eine, die sie bloß zählt, dieselbe Zahl.
Beide bestehen. Der Fehler zeigt sich beim ersten ungleichen Wert -- meist nach dem Release.

[source]
----
Beyond example-based tests, write invariant tests: properties that must hold
for every valid input, not just the ones in the specification. For each
calculation, state the invariant that connects its inputs to its output and
test it over a range of values.

Ask specifically: which of these tests would still fail if a future change
altered the units, the granularity, or the cardinality of an input?
----

Die zweite Frage ist die, die sich auszahlt.
Sie prüft die Testsuite gegen Änderungen, die die Spezifikation nie vorgesehen hat -- genau dort sind beispielbasierte Tests bauartbedingt blind.


==== Feedback-Loop zur Spezifikation

Dokumentation driftet in zwei Richtungen vom Code weg, und jede braucht ihre eigene Frage.

Sie fällt zurück, wenn die Implementierung etwas zutage fördert: eine getroffene Entscheidung, eine geklärte Regel, eine gefundene Einschränkung.
Und sie veraltet im Stillen: Ein Modul wird umbenannt, eine Spalte verschwindet, eine Fähigkeit bleibt beschrieben, die nie gebaut wurde.
Die zweite Sorte übersteht die Frage nach dem Gelernten, denn gelernt wurde nichts -- der Satz war schon falsch, als man zuletzt darüber hinweggelesen hat.

Regelmäßig die KI nach beidem fragen:

[source]
----
Two questions about the specification and architecture documentation.

First: based on what we learned during implementation, is anything missing
or now wrong?

Second: verify the documents against the code. For each structural claim,
confirm that it holds today: module names, class names, file paths, table
and column names, enum values, invariants, start commands, and claims of
the form "the only place where X happens".

List every claim that no longer holds, with document location and code
location. Do not change the code to match the document. Either correct the
document, or open an issue where the code is wrong.

Report the number of claims checked and the number that drifted.
----
...
Das Verbot in der zweiten Frage trägt Gewicht.
Das Dokument beschreibt eine Absicht, der Code die Wirklichkeit; wer den Code an das Dokument anpasst, macht aus einem Dokumentationsfehler eine stille Verhaltensänderung.

Strukturelle Aussagen lassen sich zusätzlich mechanisch prüfen -- billiger und verlässlicher, als jedes Mal eine KI zu fragen:

[source]
----
Extend the traceability check so that structural claims in the documentation
are verified mechanically: table names, column names, module paths, enum
values. A claim about structure that no test can verify does not belong in
the documentation. Either make it verifiable or delete it.
----

Traceability-Werkzeuge laufen üblicherweise nur vorwärts: Sie erzwingen, dass jede Regel einen Test hat.
Diese Richtung bemerkt nie eine dokumentierte Spalte, die es im Schema nicht gibt.
...
Wenn der Loop endet, die beiden Dokumentations-Fragen aus Schritt 8 noch einmal stellen -- gegen den Code, der jetzt dasteht.
Hier zahlt sie sich am meisten aus: Bugfixes ändern Verhalten, und geändertes Verhalten macht aus einer korrekten strukturellen Aussage eine veraltete.
Vor dem Release, nicht danach -- sonst findet der erste Leser der Dokumentation die Drift.
