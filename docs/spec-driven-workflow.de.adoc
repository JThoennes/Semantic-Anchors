...
Die Eigenschaft hinter der Regel prüfen sie nicht.
Eine Regel wie „alle Anteile sind gleich“ macht Gewichtung unsichtbar: Ein halber Anteil, der voll statt anteilig zählt, liefert eine plausible Zahl, und jeder beispielbasierte Test bleibt grün.
...
==== Dokumentation gegen den Code verifizieren

Der Feedback-Loop oben stellt eine vorwärtsgerichtete Frage: Was haben wir gelernt?
Eine KI, die nichts Neues gelernt hat, antwortet wahrheitsgemäß „nein“ -- und eine bereits falsche Aussage bleibt stehen.
Drift übersteht deshalb beliebig viele Prüfschritte.
Sie wird nicht gefunden, indem man nach Neuigkeiten fragt, sondern indem man fragt, ob eine bestimmte Aussage noch stimmt.

Also die Richtung umkehren:

[source]
----
Verify the architecture and specification documents against the code.
For each structural claim, confirm that it holds today: module names, class
names, file paths, table and column names, enum values, invariants, start
commands, and claims of the form "the only place where X happens".
...
Das Verbot im mittleren Absatz trägt das Gewicht.
...
Wenn der Loop endet, die Dokumentations-Verifikation aus Schritt 8 noch einmal laufen lassen -- gegen den Code, der jetzt dasteht.
