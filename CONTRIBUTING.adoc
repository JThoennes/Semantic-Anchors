...
* `:tier:` - Activation tier: `1` (frontier-only), `2` (needs qualification), `3` (broadly recognized). Internal: it is a judgement and is deliberately not rendered on the site
* `:prior-test:` - Date the anchor was measured with the link:#/anchor-prior-test[Anchor Prior Test]. The key into the link:#/prior-tests[prior-test register]; absent means "not measured", which is a statement about our records rather than about the term
* `:tested-on:` - Every model the anchor was measured against, with its result, as `model=yes|no` pairs (e.g. `haiku-4.5=no, sonnet-5=yes, opus-5=yes`). Rendered in the modal. List *all* tested models, not only the successes: a model that was tried and showed no effect is a different statement from one that was never tried, and dropping it would read as the latter
...
Omitting the section does not mean discarding the result. Record the negative finding in the link:#/prior-tests[prior-test register] with its date and the languages searched, so a later contributor can tell "researched, nothing found" from "never looked". A "nothing found" without its search frame is not evidence of absence — say what was searched and what could not be reached, and never report an unreachable source as a demonstrated absence.
...
. All required metadata attributes present (`:categories:`, `:roles:`, `:proponents:`, `:tier:`)
...
. No build artifacts in commit (e.g. `website/dist/`, generated files)
