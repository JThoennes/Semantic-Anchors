...
* `:verified-on:` - Models the anchor was shown to *work* on, comma-separated (e.g. `haiku-4.5, sonnet-5, opus-5`). Rendered in the modal. List only where the anchor delivered, never merely where it was tried — an anchor tested on three tiers but indistinguishable from a no-anchor baseline on one lists the other two
